...
> Last compiled: 2026-06-13 (child-compile 20260613-105538 — fourth verification run of the 20260611-230008 priority batch; zero raw/ changes since baseline 1b561d5a, confirmed via git log (raw/ untouched) and timestamp sweep (all 14 unique priority inputs predate their referencing wiki articles, which were last compiled 2026-06-11/12), so no recompilation needed; all 14 inputs remain sourced into wiki articles (seed-clawd-agents → projects/clawd-agent-rules + patterns; seed-workspace-agents → projects/workspace-agent-rules + governance; seed-agents-rules/seed-server-state → architecture+concepts; certbot-acme-fix → troubleshooting/nginx-certbot-acme-fix; ssh-normalization/wrapper-recursion/autofinish/no-pager research+learnings → troubleshooting articles); no article content or _concepts.md changes this run; deferred: seed-progress-history.md (historical log, already sourced, no new content) and the non-priority compile candidates (machine-generated digests/state snapshots/session summaries/changelogs — provenance records consumed by wiki-manager stable state pages, not compile targets); out-of-scope index gap noted for lint/wiki-manager: projects/agents-plugin-ecosystem, projects/ned-autonomous, projects/slimy-discord-bot absent from this curated main index (slimy-discord-bot already in _weak-links.md))
