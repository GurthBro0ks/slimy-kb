# Orphaned Pages

> Pages with zero inbound links from other non-index wiki pages.
> Generated: 2026-04-09T20:57:40Z by kb-lint.sh

**Total orphans: 15**

## Likely Parent Pages

- `architecture/nuc2-server-state.md`
- `log.md`
- `projects/actionbook.md`
- `projects/agents-backup-full.md`
- `projects/apify-market-scanner.md`
- `projects/kalshi-ai-trading-bot.md`
- `projects/mailbox-outbox.md`
- `projects/mcp-agent-mailbox.md`
- `projects/ned-clawd.md`
- `projects/nuc1-project-anomalies.md`
- `projects/obsidian-vault-automation.md`
- `projects/repo-health-overview.md`
- `projects/workspace-executor.md`
- `projects/workspace-researcher.md`
- `troubleshooting/nuc1-wrapper-recursion-fix.md`

## All Orphaned Pages
- `architecture/nuc2-server-state.md`
- `log.md`
- `projects/actionbook.md`
- `projects/agents-backup-full.md`
- `projects/apify-market-scanner.md`
- `projects/kalshi-ai-trading-bot.md`
- `projects/mailbox-outbox.md`
- `projects/mcp-agent-mailbox.md`
- `projects/ned-clawd.md`
- `projects/nuc1-project-anomalies.md`
- `projects/obsidian-vault-automation.md`
- `projects/repo-health-overview.md`
- `projects/workspace-executor.md`
- `projects/workspace-researcher.md`
- `troubleshooting/nuc1-wrapper-recursion-fix.md`
