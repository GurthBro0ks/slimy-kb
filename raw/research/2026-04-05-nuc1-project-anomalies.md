---
description: Anomalies and discrepancies found during NUC1 project discovery
type: reference
---

# NUC1 Project Discovery — Anomalies and Discrepancies
...
- mailbox_ingest may be the NUC2-side ingest process (and is referenced in NUC2's systemd units as `nuc-mailbox-ingest.service`)
- The cross-NUC flow is: NUC1 mailbox_outbox (pushes) → NUC2 mailbox.git → NUC2 mailbox_ingest (processes)
...
- No cron, PM2, systemd, or Docker references to this path
- openclaw.json does not reference it
- sync-repos.sh does not reference it

**Possible explanation:**
- qoder tool may have created its own clone
- Or this is a staging area for qoder-based development

**Action needed:** Verify with qoder tooling or agent if this path is actively used. Could be stale and safe to remove.
...
- GitHub: GurthBro0ks/kalshi-ai-trading-bot (presumed)
...
**Context:**
- pm_updown_bot_bundle is the active trading automation (20+ cron entries, current commits)
- kalshi-ai-trading-bot appears to be a prototype that was superseded
...
**What it is:**
- slimyai-web (GurthBro0ks/slimyai-web) runs on NUC2, not NUC1
- slimyAI.xyz/web is served from NUC2 port 3000
- NUC1's port 3000 serves slimy-bot-v2 (the Discord bot), not the web app

**Not an error** — per architecture/cross-nuc-communication-matrix.md, this is expected NUC placement.
