# NUC2 Repo Digests — 2026-04-30T00:42:09Z

**Host:** slimy-nuc2
**Timestamp:** 2026-04-30T00:42:09Z

## agents-backup-full
- **Path:** `/home/slimy/agents-backup-full`
- **Status:** PATH NOT FOUND

## clawd
- **Path:** `/home/slimy/clawd`
- **Git:** True
- **Branch:** main
- **Commit:** 858b799
- **Subject:** chore: compound nightly review Apr 29 - 29d silence, NUC1 23d down
- **Dirty:** True
- **Origin:** git@github.com:GurthBro0ks/clawd.git

## git-notes-ledger
- **Path:** `/home/slimy/.openclaw/memory/git-notes-ledger`
- **Git:** True
- **Branch:** master
- **Commit:** e1d63a6
- **Subject:** docs: auto-sync project docs from slimy-nuc2 2026-04-10
- **Dirty:** False

## kb
- **Path:** `/home/slimy/kb`
- **Git:** True
- **Branch:** main
- **Commit:** 6e51fd7
- **Subject:** kb: maintenance run 2026-04-30-0042 from slimy-nuc2
- **Dirty:** True
- **Origin:** git@github.com:GurthBro0ks/slimy-kb.git

## mailbox
- **Path:** `/home/slimy/nuc-comms/mailbox.git`
- **Git:** False
- **.service or .timer in ~/.config/systemd/user:** True

## mailbox_ingest
- **Path:** `/home/slimy/nuc-comms/mailbox_ingest`
- **Git:** True
- **Branch:** main
- **Commit:** c034217
- **Subject:** docs: auto-sync project docs from slimy-nuc2 2026-04-10
- **Dirty:** False
- **Origin:** /home/slimy/nuc-comms/mailbox.git

## mcp_agent_mail
- **Path:** `/home/slimy/.mcp_agent_mail_git_mailbox_repo`
- **Git:** True
- **Branch:** master
- **Commit:** 6fb350b
- **Subject:** docs: auto-sync project docs from slimy-nuc2 2026-04-10
- **Dirty:** False

## mission-control
- **Path:** `/home/slimy/mission-control`
- **Git:** True
- **Branch:** main
- **Commit:** 71695ee
- **Subject:** fix: add missing CREATE TABLE IF NOT EXISTS for ops database schema
- **Dirty:** False
- **Origin:** git@github.com:GurthBro0ks/mission-control.git
- **Name:** mission-control
- **Version:** 0.1.0

## nuc-comms
- **Path:** `/home/slimy/nuc-comms`
- **Git:** False
- **.service or .timer in ~/.config/systemd/user:** True

## pm_updown_bot_bundle
- **Path:** `/home/slimy/pm_updown_bot_bundle`
- **Git:** True
- **Branch:** main
- **Commit:** 29921b0
- **Subject:** docs: auto-sync project docs from slimy-nuc2 2026-04-10
- **Dirty:** False
- **Origin:** git@github.com:GurthBro0ks/pm_updown_bot_bundle.git

## slimy-monorepo
- **Path:** `/home/slimy/slimy-monorepo`
- **Git:** True
- **Branch:** main
- **Commit:** 55d14ea
- **Subject:** web-phase4: add 404 page, standardize placeholders, nav audit — web audit complete
- **Dirty:** False
- **Origin:** git@github.com:GurthBro0ks/slimy-monorepo.git
- **Name:** slimy-monorepo
- **Version:** ?

## slimyai-web
- **Path:** `/home/slimy/slimy-web`
- **Status:** PATH NOT FOUND

## workspace
- **Path:** `/home/slimy/.openclaw/workspace`
- **Git:** True
- **Branch:** master
- **Commit:** b564ec0
- **Subject:** docs: auto-sync project docs from slimy-nuc2 2026-04-10
- **Dirty:** False

## Summary

- Tracked repos: 13
- Present: 11
- Missing: 2
