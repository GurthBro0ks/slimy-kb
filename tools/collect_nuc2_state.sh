...

{
  ...
  echo ""
  systemctl --user list-units --type=service --state=running --no-pager 2>/dev/null | grep -E '\.(service|timer)$' | head -20 || echo "- (none)"
  echo ""
  ...
  echo ""
  echo "## KB Maintenance Timer"
  echo ""
  systemctl --user list-timers --all --no-pager 2>/dev/null | grep kb-maintenance || echo "- kb-maintenance.timer: inactive"
  echo ""
  echo "## Active PM2 Processes"
  echo ""
  if command -v pm2 &>/dev/null; then
    pm2 list 2>/dev/null | grep -E 'online|errored|stopped' | head -20 || echo "- (none)"
  else
  ...
  echo ""
  ss -tlnp 2>/dev/null | grep -E ':(3000|3838|3850|18790|18791|18792|18793|3307|5432|443|80)' || echo "- (ss unavailable)"
  echo ""
  echo "## Disk Usage (KB-relevant paths)"
  echo ""
  for path in "/home/slimy/kb" "/home/slimy"; do
    if [[ -d "$path" ]]; then
      du -sh "$path" 2>/dev/null || echo "- $path: (unable to measure)"
    fi
  done
  echo ""
  echo "## Uptime"
  echo ""
  uptime || echo "- (unavailable)"
  echo ""
  echo "## KB Git Status"
  echo ""
  if [[ -d /home/slimy/kb/.git ]]; then
    git -C /home/slimy/kb status --short 2>/dev/null | head -10 || echo "- (no output)"
    ...
    echo "- ahead: ${git_ahead:-0}"
    echo "- behind: ${git_behind:-0}"
  else
    echo "- (not a git repo)"
  fi
  echo ""
  ...
  echo ""
  if command -v pm2 &>/dev/null; then
    pm2 describe obsidian-headless-sync 2>/dev/null | grep -E 'status|uptime' | head -3 || echo "- obsidian-headless-sync: not tracked"
  fi
} > "$OUTPUT_FILE"
