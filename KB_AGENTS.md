...
## Page Metadata (git-backed)

Every canonical wiki page has a machine-managed metadata block derived from git history. This block is inserted after the header blockquotes and is visible in Obsidian.
...
**Last edited** = date and time of the latest git commit that touched the page file (ISO format, UTC).
**Version** = per-page revision count (`rN`) and short commit SHA of the latest page change.

This metadata is:
- Generated by `tools/kb-apply-metadata.py`
- Refreshed automatically during `kb-maintenance.sh` (every 12h) and `wiki_manager_stage1.py` runs
- Idempotent (re-running does not duplicate blocks)
- Visible in Obsidian via the vault mirror
- Not a replacement for the human-maintained `> Updated:` header field

Manual refresh: `wiki metadata` or `python3 tools/kb-apply-metadata.py`
...
- `wiki metadata [--dry-run] [--page <path>]` — refresh git-backed last-edited/version metadata on all wiki pages
